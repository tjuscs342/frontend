...
export function setSearchKey(value) {
  return {
    type: 'DATA_SET@PUSH_PROMOTION@SEARCH',
    value
  }
}
...
function deleteWaiting(id) {
  return {
    type: 'DATA_SET@PUSH_PROMOTION@DELETE_WAITING',
    id
  }
}
...
function deleteDone(data, id) {
  return {
    type: 'DATA_SET@PUSH_PROMOTION@DELETE_DONE',
    id,
    data
  }
}
...
function forbidOperationAndMockTable(value) {
  return {
    type: 'DATA_SET@PUSH_PROMOTION@FORBID_OPERATION_AND_MOCK_TABLE',
    value
  }
}
/**
...
function allowOperationAndDeleteMockTable(value) {
  return {
    type: 'DATA_SET@PUSH_PROMOTION@ALLOW_OPERATION_AND_DELETE_MOCK_TABLE',
    value
  }
}
/**
...
export function setCopyStatus(id, status) {
  return {
    type: 'DATA_SET@PUSH_PROMOTION@SET_COPY_STATUS',
    id,
    status
  }
}
...
export function loadTableData() {
  return (dispatch/* , getState*/) => (
    fetchPro(api('myDataSet:pushPromotionSet', getUser().name)) // 'myDataSet:predefinedUserSet'
      .then(response => response.json())
      .catch(() => ({ status: 'fail', result: [] }))
      .then(json => dispatch({
        type: 'DATA_SET@PUSH_PROMOTION@TABLE_LOAD',
        data: json
      }))
  )
}
...
export function setSelectedOperation(operation) {
  return (dispatch, getState) => {
    const { mockTable, couldOperate, selectedRows, table } = getState().myDataSet.pushPromotion.toJS()
    ...
    fetchPro(api('myDataSet:pushPromotionOperation'), {
      method: 'POST',
      body: formData
    })
      .then(response => response.json())
      ...
      .then(json => {
        ...
        const { mockTable, couldOperate, selectedRows } = getState().myDataSet.pushPromotion.toJS()
...
export function recount(oldLine) {
  return (dispatch, getState) => {
    const { table, mockTable } = getState().myDataSet.pushPromotion.toJS()
    // 请求所需参数
    const formData = new FormData()
    ...
    // 用假数据更新store
    dispatch({
      type: 'DATA_SET@PUSH_PROMOTION@DO_RECOUNT',
      value
    })

    fetchPro(api('myDataSet:pushPromotionRecount'), {
      method: 'POST',
      body: formData
    })
      .then(response => response.json())
      ...
      .then(json => {
        ...
        switch (json.status) {
          case 'succ':
            dispatch({
              type: 'DATA_SET@PUSH_PROMOTION@RECOUNT_DONE',
              id: oldLine.id
            })
            dispatch(loadTableData())
            break
...
export function deleteLine(id) {
  ...
  return (dispatch) => {
    ...
    fetchPro(api('myDataSet:pushPromotionDelete'), {
      method: 'POST',
      body: bodyData
    })
      .then(response => response.json())
...
export function setSelectedRow(selectionType, value) {
  return (dispatch, getState) => {
    const { selectedRows, mockTable, couldOperate } = getState().myDataSet.pushPromotion.toJS()
    // 选择框操作改变的store状态
    const data = rowSelectionWorker(selectionType, selectedRows, value, mockTable, couldOperate)
    dispatch({
      type: 'DATA_SET@PUSH_PROMOTION@ROW_SELECTION',
      data
    })
  }
}
