...
const reducerMap = {
  'DATA_SET@USER_PROFILE@TABLE_LOAD': (state, action) => {
  ...
  'DATA_SET@USER_PROFILE@SEARCH': (state, action) => {
    return state.set('searchKey', Immutable.fromJS(action.value))
  },
  'DATA_SET@USER_PROFILE@DELETE_WAITING': (state, action) => {
  ...
  'DATA_SET@USER_PROFILE@DELETE_DONE': (state, action) => {
  ...
  'DATA_SET@USER_PROFILE@ROW_SELECTION': (state, action) => {
  ...
  'DATA_SET@USER_PROFILE@FORBID_OPERATION_AND_MOCK_TABLE': (state, action) => {
  ...
  'DATA_SET@USER_PROFILE@ALLOW_OPERATION_AND_DELETE_MOCK_TABLE': (state, action) => {
    const { couldOperate, mockTable } = action.value
    return state.set('couldOperate', Immutable.fromJS(couldOperate))
                .set('mockTable', Immutable.fromJS(mockTable))
  },
  'DATA_SET@USER_PROFILE@DO_RECOUNT': (state, action) => {
    const { table, mockTable } = action.value
    return state.set('table', Immutable.fromJS(table))
                .set('mockTable', Immutable.fromJS(mockTable))
  },
  'DATA_SET@USER_PROFILE@RECOUNT_DONE': (state, action) => {
    return state.update('mockTable', (oldArray) => Immutable.fromJS(oldArray.toJS().filter(x => x.id !== action.id)))
  },
  'DATA_SET@USER_PROFILE@SET_COPY_STATUS': (state, action) => {
